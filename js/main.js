...
$(document).ready(function() {
  ...
  if (window.location.search == "?easy") pipeheight = 150;
  ...
  $("#saveplayer").click(function() {
    var values = { playername: $("#playername").val(), score: score };
    $.ajax({
      url: "save-player.php",
      type: "post",
      data: values,
...
function showSplash() {
  ...
  // $("#topscorers").transition({ opacity: 1 }, 2000, 'ease');
  // $("#topscorers").show();
}
...
function startGame() {
  ...
  // $("#topscorers").stop();
  // $("#topscorers").transition({ opacity: 0 }, 500, 'ease');
  // $("#topscorers").hide();
...
function showScore() {
  ...
  //show the scoreboard
  $("#scoreboard").css({ y: "25%", opacity: 0 }); //move it down so we can slide it up
  $("#replay").css({ y: "25%", opacity: 0 });
  $("#scoreboard").transition(
    ...
    function() {
      ...
      soundSwoosh.stop();
      soundSwoosh.play();
      $("#entername").transition(
        { y: "0px", opacity: 1, display: "block" },
        600,
        "ease"
      );
